...
open class ITPhysicsPanHandler {
    ...
    public final class ITState {
        ...
        var snapshots: [ITMotionSnapshot] = []
        ...
        func addSnapshot() {
            ...
            snapshots.append(ITMotionSnapshot(angle: angle, time: time))
        }
    ...
    weak var animator: ITAnimator?
    weak var messageView: UIView?
    weak var containerView: UIView?
    private(set) public var state: ITState?
    ...
    func configure(context: ITAnimationContext, animator: ITAnimator) {
        if let oldView = (messageView as? ITBackgroundViewable)?.backgroundView ?? messageView {
            oldView.removeGestureRecognizer(pan)
        }
        messageView = context.messageView
        let view = (messageView as? ITBackgroundViewable)?.backgroundView ?? messageView
    ...
    @objc func pan(_ pan: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            let state = ITState(messageView: messageView, containerView: containerView)
            self.state = state
            let center = messageView.center
