...
/*
 */
open class ITMessageView: ITBaseView, ITIdentifiable, ITAccessibleMessage, WKNavigationDelegate {
    ...
    open var onTapDirecLink: (() -> Void)?
    func onTappedDirectLink(onTapped: (() -> Void)?) {
        self.onTapDirecLink = onTapped
    }
    ...
    @IBOutlet open var button: UIButton? {
        didSet {
            if let old = oldValue {
                old.removeTarget(self, action: #selector(ITMessageView.buttonTapped(_:)), for: .touchUpInside)
            }
            if let button = button {
                button.addTarget(self, action: #selector(ITMessageView.buttonTapped(_:)), for: .touchUpInside)
            }
        }
    }
...
extension ITMessageView {
     ...
     */
    public static func viewFromNib<T: ITMessageView>(layout: Layout, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! ITSwiftMessages.viewFromNib(named: layout.rawValue)
    }
    ...
    /**
     ...
     - Returns: An instance of generic view type `T: MessageView`.
     */
    public static func viewFromNib<T: ITMessageView>(layout: Layout, bundle: Bundle, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! ITSwiftMessages.viewFromNib(named: layout.rawValue, bundle: bundle, filesOwner: filesOwner)
    }
}
...
extension ITMessageView {
    ...
    func configureContent(campaign: ANCampaign, buttonTapHandler: ((_ button: UIButton) -> Void)?) {
        contentWebView.navigationDelegate = self
        contentWebView.scrollView.isScrollEnabled = false
        if campaign.content.count > 0 {
            let fullAdsHTML = NSMutableString()
            let stringContent = campaign.content.replacingOccurrences(of: "\\", with: "")
            //stringContent = stringContent.replacingOccurrences(of: "//", with: "https://")
            fullAdsHTML.append(stringContent)
            fullAdsHTML.append("<style>")
            fullAdsHTML.append(campaign.css)
            fullAdsHTML.append("</style>")
            fullAdsHTML.append("<script>")
            fullAdsHTML.append(campaign.javascript)
            fullAdsHTML.append("</script>")
            contentWebView.loadHTMLString(fullAdsHTML as String, baseURL: nil)
        }
        self.buttonTapHandler = buttonTapHandler
    }
