...
protocol ITPresenterDelegate: ITAnimationDelegate {
    func hide(presenter: ITPresenter)
}

class ITPresenter: NSObject {

    // MARK: - API
    init(config: ITSwiftMessages.ITConfig, view: UIView, delegate: ITPresenterDelegate) {
        self.config = config
        self.view = view
        self.delegate = delegate
        self.animator = ITPresenter.animator(forPresentationStyle: config.presentationStyle, delegate: delegate)
        
        if let identifiable = view as? ITIdentifiable {
            id = identifiable.id
        } else {
    ...
    var id: String
    var config: ITSwiftMessages.ITConfig
    let maskingView = ITMaskingView()
    let animator: ITAnimator
    var isHiding = false
    let view: UIView
    ...
    enum ITPresentationContext {
        case viewController(_: ITWeak<UIViewController>)
        case view(_: ITWeak<UIView>)
    ...
    private weak var delegate: ITPresenterDelegate?
    private var presentationContext = ITPresentationContext.viewController(ITWeak<UIViewController>(value: nil))
    ...
    private static func animator(forPresentationStyle style: ITSwiftMessages.ITPresentationStyle, delegate: ITAnimationDelegate) -> ITAnimator {
        switch style {
        case .top:
            return ITTopBottomAnimation(style: .top, delegate: delegate)
        case .bottom:
            return ITTopBottomAnimation(style: .bottom, delegate: delegate)
        case .center:
            return ITPhysicsAnimation(placement: .center, delegate: delegate)
        case .custom(let animator):
            animator.delegate = delegate
            return animator
        case .fullscreen:
            return ITPhysicsAnimation(placement: .fullscreen, delegate: delegate)
        }
    }

    func show(completion: @escaping ITAnimationCompletion) throws {
    ...
    private func showAnimation(completion: @escaping ITAnimationCompletion) {
    ...
    private func showAccessibilityAnnouncement() {
        guard let accessibleMessage = view as? ITAccessibleMessage,
    ...
    func hide(animated: Bool, completion: @escaping ITAnimationCompletion) {
        isHiding = true
        self.config.eventListeners.forEach { $0(.willHide) }
        let context = animationContext()
        let action = {
            if let viewController = self.presentationContext.viewControllerValue() as? ITWindowViewController {
    ...
    private func animationContext() -> ITAnimationContext {
        return ITAnimationContext(messageView: view, containerView: maskingView, safeZoneConflicts: safeZoneConflicts(), interactiveHide: config.interactiveHide)
    }

    private func safeZoneConflicts() -> ITSafeZoneConflicts {
        guard let window = maskingView.window else { return [] }
        let windowLevel: UIWindow.Level = {
            if let vc = presentationContext.viewControllerValue() as? ITWindowViewController {
                return vc.windowLevel
            }
            return UIWindow.Level.normal //UIWindow.Level.normal
        }()
        ...
        if #available(iOS 11, *) {
            if windowLevel > UIWindow.Level.normal {
                ...
                if maskingView.safeAreaInsets.top == 20 {
                    ...
                    var conflicts: ITSafeZoneConflicts = []
            ...
            return statusBarViewFrame.intersects(maskingView.bounds) ? ITSafeZoneConflicts.statusBar : []
            #endif
        }
    }

    private func getPresentationContext() throws -> ITPresentationContext {

        func newWindowViewController(_ windowLevel: UIWindow.Level) -> UIViewController {
            let viewController = ITWindowViewController.newInstance(windowLevel: windowLevel, config: config)
        ...
        case .automatic:
            ...
            if let rootViewController = UIApplication.shared.keyWindow?.rootViewController {
                ...
                return .viewController(ITWeak(value: viewController))
            } else {
                throw ITSwiftMessagesError.noRootViewController
            }
            #endif
    ...
    func install() {
        ...
        func installAccessibility() {
            ...
            if let accessibleMessage = view as? ITAccessibleMessage {
            ...
            if config.dimMode.interactive {
                ...
                dismissView.accessibilityTraits = UIAccessibilityTraits.button //UIAccessibilityTraits.button
        ...
        if let windowViewController = presentationContext.viewControllerValue() as? ITWindowViewController {
            if #available(iOS 13, *) {
