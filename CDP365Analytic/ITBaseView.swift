 ...
 */
open class ITBaseView: UIView, ITBackgroundViewable, ITMarginAdjustable {
    ...
    /**
     Fulfills the `ITBackgroundViewable` protocol and is the target for
     ...
     */
    open var tapHandler: ((_ view: ITBaseView) -> Void)? {
        didSet {
            installTapRecognizer()
        }
    }
    ...
    fileprivate lazy var tapRecognizer: UITapGestureRecognizer = {
        let tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(ITMessageView.tapped))
