...
private let globalInstance = ITSwiftMessages()
...
open class ITSwiftMessages {
    ...
    */
    public enum ITPresentationStyle {
        ...
        */
        case custom(animator: ITAnimator)
    }
    ...
    */
     ...
     */
    public typealias ITEventListener = (ITEvent) -> Void
    ...
    public struct ITConfig {
         ...
         */
        public var presentationStyle = ITPresentationStyle.top

        /**
         Specifies how the container for presenting the message view
         is selected. The default is `.Automatic`.
         */
        public var presentationContext = ITPresentationContext.automatic

        /**
         Specifies the duration of the message view's time on screen before it is
         automatically hidden. The default is `.Automatic`.
         */
        public var duration = ITDuration.automatic
        
        /**
         Specifies options for dimming the background behind the message view
         similar to a popover view controller. The default is `.None`.
         */
        public var dimMode = ITDimMode.none
        ...
        */
        public var eventListeners: [ITEventListener] = []
        ...
        */
         ...
         */
        public var windowViewController: ((_ windowLevel: UIWindow.Level?, _ config: ITSwiftMessages.ITConfig) -> ITWindowViewController)?

        /**
         Supply an instance of `KeyboardTrackingView` to have the message view avoid the keyboard.
         */
        public var keyboardTrackingView: ITKeyboardTrackingView?
    }
     ...
     */
    open func show(config: ITConfig, view: UIView) {
        let presenter = ITPresenter(config: config, view: view, delegate: self)
        messageQueue.sync {
    ...
    public typealias ITViewProvider = () -> UIView
     ...
     */
    open func show(config: ITConfig, viewProvider: @escaping ITViewProvider) {
    ...
    /**
     ...
     - Parameter viewProvider: A block that returns the view to be displayed.
     */
    public func show(viewProvider: @escaping ITViewProvider) {
     ...
     */
    public var defaultConfig = ITConfig()
    ...
    fileprivate class ITDelays {

        fileprivate var ids = Set<String>()

        fileprivate func add(presenter: ITPresenter) {
            ids.insert(presenter.id)
        }

        @discardableResult
        fileprivate func remove(presenter: ITPresenter) -> Bool {
    ...
    func show(presenter: ITPresenter) {
        messageQueue.sync {
            enqueue(presenter: presenter)
        }
    }
    ...
    fileprivate var queue: [ITPresenter] = []
    fileprivate var delays = ITDelays()
    fileprivate var counts: [String : Int] = [:]
    fileprivate var _current: ITPresenter? = nil {
        didSet {
            if oldValue != nil {
    ...
    fileprivate func enqueue(presenter: ITPresenter) {
...
extension ITSwiftMessages: ITPresenterDelegate {
    ...
    public func hide(animator: ITAnimator) {
        messageQueue.sync {
    ...
    public func panStarted(animator: ITAnimator) {
        autohideToken = nil
    }

    public func panEnded(animator: ITAnimator) {
        queueAutoHide()
    }

    private func presenter(forAnimator animator: ITAnimator) -> ITPresenter? {
...
extension ITSwiftMessages {
    ...
    fileprivate class func internalViewFromNib<T: UIView>(named name: String, bundle: Bundle? = nil, filesOwner: AnyObject = NSNull.init()) throws -> T {
        ...
        #if swift(>=4.1)
        guard let view = arrayOfViews.compactMap( { $0 as? T} ).first else { throw ITSwiftMessagesError.cannotLoadViewFromNib(nibName: name) }
        #else
        guard let view = arrayOfViews.flatMap( { $0 as? T} ).first else { throw ITSwiftMessagesError.cannotLoadViewFromNib(nibName: name) }
        #endif
        return view
    }
}
...
extension ITSwiftMessages {
     ...
     */
    public static var sharedInstance: ITSwiftMessages {
        return globalInstance
    }
    
    public static func show(viewProvider: @escaping ITViewProvider) {
        globalInstance.show(viewProvider: viewProvider)
    }
    
    public static func show(config: ITConfig, viewProvider: @escaping ITViewProvider) {
    ...
    public static func show(config: ITConfig, view: UIView) {
    ...
    public static var defaultConfig: ITConfig {
        get {
