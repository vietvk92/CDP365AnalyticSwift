...
open class ITSwiftMessagesSegue: UIStoryboardSegue {
    ...
    public var presentationStyle: ITSwiftMessages.ITPresentationStyle {
    ...
    public var dimMode: ITSwiftMessages.ITDimMode {
        get { return messenger.defaultConfig.dimMode}
        set { messenger.defaultConfig.dimMode = newValue }
    }
    
    // duration
    public var duration: ITSwiftMessages.ITDuration {
    ...
    public var eventListeners: [ITSwiftMessages.ITEventListener] {
     ...
     */
    public var messageView = ITBaseView()
    ...
    */
    public var containerView: ITCornerRoundingView = ITCornerRoundingView()
    ...
    private var messenger = ITSwiftMessages()
    private var selfRetainer: ITSwiftMessagesSegue? = nil
    private lazy var hider = { return ITTransitioningDismisser(segue: self) }()

    private lazy var presenter = {
        return ITPresenter(config: messenger.defaultConfig, view: messageView, delegate: messenger)
    }()
...
extension ITSwiftMessagesSegue {
    ...
    public func configure(layout: ITLayout) {
        ...
        case .topMessage:
            ...
            let animation = ITTopBottomAnimation(style: .top)
            animation.springDamping = 1
            presentationStyle = .custom(animator: animation)
        case .bottomMessage:
            messageView.layoutMarginAdditions = UIEdgeInsets(top: 20, left: 20, bottom: 20, right: 20)
            messageView.collapseLayoutMarginAdditions = false
            let animation = ITTopBottomAnimation(style: .bottom)
            animation.springDamping = 1
            presentationStyle = .custom(animator: animation)
        case .topCard:
...
extension ITSwiftMessagesSegue: UIViewControllerTransitioningDelegate {
    public func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let shower = ITTransitioningPresenter(segue: self)
...
extension ITSwiftMessagesSegue {
    private class ITTransitioningPresenter: NSObject, UIViewControllerAnimatedTransitioning {

        fileprivate private(set) var completeTransition: ((Bool) -> Void)?
        private weak var segue: ITSwiftMessagesSegue?

        fileprivate init(segue: ITSwiftMessagesSegue) {
            self.segue = segue
        }
...
extension ITSwiftMessagesSegue {
    private class ITTransitioningDismisser: NSObject, UIViewControllerAnimatedTransitioning {
