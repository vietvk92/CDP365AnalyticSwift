...
public class ITTopBottomAnimation: NSObject, ITAnimator {
    ...
    public init(style: ITStyle) {
        self.style = style
    }

    init(style: ITStyle, delegate: ITAnimationDelegate) {
        self.style = style
    ...
    func install(context: ITAnimationContext) {
        ...
        self.context = context
        if let adjustable = context.messageView as? ITMarginAdjustable {
        ...
        if context.interactiveHide {
            if let view = view as? ITBackgroundViewable {
        ...
        if let view = view as? ITBackgroundViewable,
            let cornerRoundingView = view.backgroundView as? ITCornerRoundingView,
    ...
    func showAnimation(completion: @escaping ITAnimationCompletion) {
    ...
    @objc func pan(_ pan: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            if !closing {
                ...
                if let background = (messageView as? ITBackgroundViewable)?.backgroundView, background != view {
                    switch style {
                    case .top:
